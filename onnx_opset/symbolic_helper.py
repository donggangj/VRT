...
def _get_const(value, desc, arg_name):
    if not _is_constant(value):
        raise RuntimeError(
            "ONNX symbolic expected a constant value of the {} argument, got `{}`".format(arg_name, value))
    return _parse_arg(value, desc)
...
def _onnx_opset_unsupported_detailed(op_name, current_opset, supported_opset, reason):
    raise RuntimeError("Unsupported: ONNX export of {} in "
                       "opset {}. {}. Please try opset version {}.".format(op_name, current_opset, reason,
                                                                           supported_opset))
...
def _interpolate_warning(interpolate_mode):
    ...
    warnings.warn(
        "You are trying to export the model with " + onnx_op + " for ONNX opset version " + str(
...
def _reducesum_helper(g, input, axes_i=None, keepdims_i=1, noop_with_empty_axes_i=0):
    ...
    if _export_onnx_opset_version >= 13:
        if axes_i:
            ...
            return g.op("ReduceSum", input, axes_i, keepdims_i=keepdims_i,
