...
def unsafe_chunk(g, self, chunks, dim, _outputs=None):
    if _outputs is None:
        return sym_help._onnx_opset_unsupported_detailed("unsafe_chunk", 9, 11,
                                                         "Dynamic number of outputs not supported")
    size = sym_help._get_tensor_dim_size(self, dim)
    if size is None:
...
def split_with_sizes(g, self, split_sizes, dim, _outputs=None):
    if not sym_help._is_split_static(split_sizes, _outputs):
        return sym_help._onnx_opset_unsupported_detailed("split_with_sizes", 9, 11,
                                                         "Dynamic number of outputs not supported")
...
def conv_transpose1d(g, input, weight, bias, stride, padding, output_padding, groups, dilation):
    return _convolution(g, input, weight, bias, stride, padding, dilation, True, output_padding, groups, None, None,
                        None, None)
...
def repeat_interleave(g, self, repeats, dim=None, output_size=None):
    ...
    for idx, r_split in enumerate(r_splits):
        ...
        i_split = sym_help._reshape_helper(g, i_split, g.op("Constant", value_t=torch.LongTensor(input_sizes)),
                                           allowzero=0)
...
def arange(g, *args):
    ...
    if len(args) == 2 or len(args) == 5:
        ...
        arange_tensor = g.op("Add",
                             sym_help._squeeze_helper(g, nonzero(g, ones(g, range_tensor, dtype, *(args[3:]))), [1]),
                             start)
        return g.op("Cast", arange_tensor, to_i=sym_help.scalar_type_to_onnx[dtype])
    else:
...
def index(g, self, index):
    ...
    if len(indices) == 1:
        ...
        if len(adv_idx_indices) == 0:
            ...
            dim_tensor_list = [
                g.op("Gather", shape_tensor, g.op("Constant", value_t=torch.LongTensor([dim])), axis_i=0) for dim in
                range(rank)
            ]

            self = g.op("Transpose", self,
...
def as_strided(g, self, sizes, strides, offset=None):
    ...
    if not sym_help._is_value(sizes):
        ...
        for i, stride in enumerate(strides):
            r_size = [1] * rank
            r_size[i] = -1
            size = select(g, sizes, g.op("Constant", value_t=torch.tensor([0])),
...
def hann_window(g, window_length, periodic=True, dtype=None, layout=None, device=None, pin_memory=None,
                requires_grad=False):
    if dtype is None:
