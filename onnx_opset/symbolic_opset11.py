...
def unbind(g, self, dim=0, _outputs=None):
    if _outputs is None:
        return g.op("SplitToSequence", self, g.op("Constant", value_t=torch.tensor(1, dtype=torch.long)), axis_i=dim,
                    keepdims_i=0)
    else:
...
def _prepare_onnx_paddings(g, input, pad):
    ...
    paddings = g.op("Concat", pad, g.op("ConstantOfShape", extension, value_t=torch.tensor([0], dtype=torch.int64)),
                    axis_i=0)
...
def prim_ConstantChunk(g, self, chunks, dim):
    return sym_help._prim_constant_chunk_helper(g, self, chunks, dim)


def chunk(g, self, chunks, dim):
    return sym_help._chunk_helper(g, self, chunks, dim)
